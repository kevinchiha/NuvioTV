...
internal fun PlaybackAvailabilityProvider(content: @Composable () -> Unit) {
    ...
    // KevBox FORK DIVERGENCE: never gate Play on the (possibly unresolved) addon list — see the
    // allowUnverifiedPlayback note in PlaybackAvailability.kt. Set here, not in the view model, so
    // upstream's PlaybackAvailabilityViewModelTest keeps its expectations.
    val kevboxAvailability = availability.copy(allowUnverifiedPlayback = true)
    CompositionLocalProvider(LocalPlaybackAvailability provides kevboxAvailability, content = content)
}
