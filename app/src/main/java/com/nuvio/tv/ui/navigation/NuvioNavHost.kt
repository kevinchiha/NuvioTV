    ...
    NavHost(
        ...
        composable(Screen.Settings.route) {
            SettingsScreen(
                ...
                // category is also hidden in SettingsScreen, so these rows never render; the no-ops are a
                // second layer in case a future merge re-shows the rows.
                onNavigateToAddons = { },
                onNavigateToPlugins = { },
        ...
        composable(Screen.AuthSignIn.route) {
            // KevBox: QR retired (its RPCs are removed server-side) — always email/password sign-in.
            // Upstream 0.8.4+ routes this to AuthQrSignInScreen after retiring SELF_HOSTED; don't take that.
            AuthSignInScreen(
                onBackPress = { navController.popBackStack() },
