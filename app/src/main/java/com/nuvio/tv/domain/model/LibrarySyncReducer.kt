...
object LibrarySyncReducer {
    ...
    fun applySnapshot(
        ...
        val pendingDeletes = current.pendingDeleteKeys.toKeyMap()
        val beforeUpserts = pendingUpserts.toMap()
        // Identities present in the remote snapshot, captured before local overlays/union
        // mutate the map (used by the KevBox first-snapshot preservation below).
        val remoteIdentities = remoteItemsByKey.keys.toSet()
        val migrateLegacyLocal =
            !current.deltaInitialized &&
                remoteItemsByKey.isEmpty() &&
                localItems.isNotEmpty()

        // KevBox FORK DIVERGENCE (ponytail): upstream's migrateLegacyLocal only preserves
        // local-only items when the first snapshot comes back EMPTY. A never-synced device
        // restoring against a NON-empty remote library dropped its local-only saved titles.
        // Union them in (remote wins on overlap) and queue them as pending upserts — mirror
        // of unionWatchedSnapshot(unionWhenNeverSynced = true) in data/local/SyncMergeLogic.kt.
        val preserveLocalOnFirstSnapshot = !current.deltaInitialized && localItems.isNotEmpty()
        ...
        val reconciledItems = if (migrateLegacyLocal) {
            ...
            if (preserveLocalOnFirstSnapshot) {
                localItems.forEach { (identity, localItem) ->
                    if (identity !in pendingDeletes) {
                        remoteItemsByKey.putIfAbsent(identity, localItem)
                    }
                }
            }
            remoteItemsByKey
        ...
        } else if (preserveLocalOnFirstSnapshot) {
            localItems.forEach { (identity, item) ->
                // Only truly local-only items: overlaps stay remote-wins and are not re-pushed.
                if (identity !in remoteIdentities && identity !in pendingUpserts && identity !in pendingDeletes) {
                    pendingUpserts[identity] = item.toLibrarySyncKey()
                }
            }
        }
