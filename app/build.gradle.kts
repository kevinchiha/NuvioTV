...
android {
    ...
    defaultConfig {
        ...
        // as a hardcoded buildConfigField default. Only the QR device-login flow reads it; we use email/password.
    ...
    buildTypes {
        debug {
            ...
            // KevBox: upstream (0.7.9) added a remote "sync backend switch" — the app fetches this
            // manifest on every onCreate/onResume/onStart and OBEYS it: it can change the active sync
            // backend and force-logout the entire fleet (forceLogoutOnChange defaults true). Upstream's
            // default points at NuvioMedia's server (https://switch.nuvioapp.space/config.json), which we
            // do NOT control. We blank it so SyncBackendRepository.refreshFromManifest() returns
            ...
            // KevBox: field must exist (ServerConfigurationStore reads it) but stays blank — upstream
            // defaults it to https://nuvio.tv/link. QR device-login is the only consumer and we don't use it.
